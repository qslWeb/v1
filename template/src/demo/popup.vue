<template>
  <div>
    ...
    <!--
      ...
      position=""	popup 的位置省略则居中显示	'top' 'right''bottom'  'left'  若不使用可以自定义样式（如本案例）

      pop-transition=""	显示/隐藏时的动效，仅在省略 position 时可配置		'popup-fade'

      modal=""	是否创建一个 modal (灰透背景)层			默认true

      closeOnClickModal=""	是否可以通过点击 modal 层来关闭 popup			默认true
...
<style>
    /* 本页面案例展示的效果与样式有关，所以保留了样式，若在使用中需要案例中的效果，同样在引用页面需要引用相关样式 */
     .mint-popup-1 {width: 200px;border-radius: 8px;padding: 10px;transform: translate(-50%, 0);}
     .mint-popup-1 h1 {font-size: 20px;color: #26a2ff;}
     .mint-popup-1 p {margin-bottom: 10px;}
     .mint-popup-1::before {
        width: 0;
        height: 0;
      ...
      .mint-popup-2 {width: 100%;height: 50px;text-align: center;background-color: rgba(0,0,0,.7);backface-visibility: hidden;}
      .mint-popup-2 p {line-height: 50px;color: #fff;}

      .mint-popup-3 {width: 100%;height: 100%;background-color: #fff;}
      .mint-popup-3 .mint-button {position: absolute; width: 90%;top: 50%;left: 5%;transform: translateY(-50%);}

      .mint-popup-4 {width: 100%;}
      .mint-popup-4 .picker-slot-wrapper, .picker-item {backface-visibility: hidden;}
